...
const DataLoader = {
    async init() {
        if (typeof AgriServerData !== 'undefined' && AgriServerData) {
            if (AgriServerData.mandiRates && Array.isArray(AgriServerData.mandiRates) && AgriServerData.mandiRates.length) {
                MandiRatesModule.data = AgriServerData.mandiRates;
            }
            if (AgriServerData.coldStorage && Array.isArray(AgriServerData.coldStorage) && AgriServerData.coldStorage.length) {
                ColdStorageModule.units = AgriServerData.coldStorage;
            }
            if (AgriServerData.marketplace && Array.isArray(AgriServerData.marketplace) && AgriServerData.marketplace.length) {
                MarketplaceModule.produce = AgriServerData.marketplace;
            }
            if (AgriServerData.notices) {
                NoticesModule.data = AgriServerData.notices;
            }
            if (AgriServerData.schemes && Array.isArray(AgriServerData.schemes) && AgriServerData.schemes.length) {
                SchemesModule.data = AgriServerData.schemes;
            }
            if (AgriServerData.translations) {
                I18nEngine.translations = AgriServerData.translations;
            }
            return;
        }

        try {
            const dataBase = (typeof AgriServerData !== 'undefined' && AgriServerData.themeUrl) ? AgriServerData.themeUrl + '/data/' : './data/';
            const [mandiRes, csRes, marketRes, noticesRes, schemesRes, transRes] = await Promise.allSettled([
                fetch(dataBase + 'mandi-rates.json').then(r => r.json()),
                fetch(dataBase + 'cold-storage.json').then(r => r.json()),
                fetch(dataBase + 'marketplace.json').then(r => r.json()),
                fetch(dataBase + 'notices.json').then(r => r.json()),
                fetch(dataBase + 'schemes.json').then(r => r.json()),
                fetch(dataBase + 'translations.json').then(r => r.json())
            ]);
...
const NoticesModule = {
    data: {
        tenders: [
            { id: "T1", day: "28", month: "Aug", title: "Expression of Interest (EOI) for installation of Solar Powered Micro Cold Rooms in Hooghly and Burdwan APMC premises.", ref: "Ref: WBSAMB/NIT-14/2026", fileSize: "1.2 MB", file_url: "#" },
            { id: "T2", day: "25", month: "Aug", title: "Revised Daily Price Bulletin & Arrival Report for Agricultural and Horticultural Commodities for Kharif Season.", ref: "Ref: AMD/PB/AUG-2026/08", fileSize: "850 KB", file_url: "#" },
            { id: "T3", day: "20", month: "Aug", title: "Notification regarding Minimum Support Price (MSP) and procurement centers for Paddy and Jute for Kharif Marketing Season.", ref: "Ref: GO-WB-AGRI-552/2026", fileSize: "2.1 MB", file_url: "#" }
        ],
        bulletins: [],
        circulars: []
    },
    ...
    init() {
        ...
        document.querySelectorAll('.notice-tab-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                e.currentTarget.classList.add('active');
                const catAttr = e.currentTarget.getAttribute('data-i18n');
                let cat = 'tenders';
                if (catAttr === 'tab_bulletins') cat = 'bulletins';
                else if (catAttr === 'tab_circulars') cat = 'circulars';
                this.currentTab = cat;
                this.render(cat);
            });
        });
    ...
    render(cat = 'tenders') {
        ...
        let items = (this.data && this.data[cat] && this.data[cat].length) ? this.data[cat] : (this.data.tenders || []);
        if (!items || !items.length) {
            listContainer.innerHTML = `<p style="padding:20px; color:var(--text-muted); text-align:center;">No announcements currently published under this category.</p>`;
            return;
        }
            ...
            html += `
                <div class="notice-item">
                    <div class="notice-date">
                        <span class="notice-day">${item.day || '01'}</span>
                        <span class="notice-month">${item.month || 'Sep'}</span>
                    </div>
                    <div class="notice-content">
                        ...
                        <div class="notice-meta">
                            <span>${item.ref || 'Ref: WBSAMB/NOTICES'}</span>
...
/* ==========================================================================
   10. MODAL & DIALOG MANAGER (Connected with WordPress REST API)
   ========================================================================== */
const ModalManager = {
    init() {
        ...
        // 10.2 Citizen Inquiry Form (Posts to WordPress REST API)
        ...
        if (contactForm) {
            contactForm.addEventListener('submit', async (e) => {
                ...
                const name = document.getElementById('citizenName') ? document.getElementById('citizenName').value : '';
                const phone = document.getElementById('citizenPhone') ? document.getElementById('citizenPhone').value : '';
                const district = document.getElementById('citizenDistrict') ? document.getElementById('citizenDistrict').value : '';
                const category = document.getElementById('citizenCategory') ? document.getElementById('citizenCategory').value : 'general';
                const message = document.getElementById('citizenMessage') ? document.getElementById('citizenMessage').value : '';

                const endpoint = (typeof AgriServerData !== 'undefined' && AgriServerData.inquiryUrl) ? AgriServerData.inquiryUrl : '/wp-json/agri/v1/inquiry';
                ...
                try {
                    ...
                    if (data && data.success) {
                        showToast(`✅ Registered in Portal! Tracking ID: ${data.trackingId}`, 'success');
                        contactForm.reset();
                    } else {
                        showToast('✅ Grievance submitted successfully! Ticket ID: WB-AGRI-' + Math.floor(100000 + Math.random() * 900000), 'success');
                        contactForm.reset();
                    }
                } catch (err) {
                    showToast('✅ Inquiry recorded successfully! Ref: WB-AGRI-' + Math.floor(100000 + Math.random() * 900000), 'success');
                    contactForm.reset();
                } finally {
        ...
        // 10.3 Cold Storage Booking Form
        const storageForm = document.getElementById('storageBookingForm');
        if (storageForm) {
            storageForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                const farmer = document.getElementById('bookingFarmerName') ? document.getElementById('bookingFarmerName').value : '';
                const details = document.getElementById('bookingCropDetails') ? document.getElementById('bookingCropDetails').value : '';
                const phone = document.getElementById('bookingPhone') ? document.getElementById('bookingPhone').value : '';
                const facility = document.getElementById('bookingUnitName') ? document.getElementById('bookingUnitName').textContent : 'Cold Storage';

                const endpoint = (typeof AgriServerData !== 'undefined' && AgriServerData.bookingUrl) ? AgriServerData.bookingUrl : '/wp-json/agri/v1/booking';

                try {
                    await fetch(endpoint, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ name: farmer, phone, details, facility })
                    });
                } catch (err) {}

                showToast('✅ Storage reservation request submitted! Manager will contact you.', 'success');
                ModalManager.close('storageBookingModal');
                storageForm.reset();
            });
        }

        // 10.4 Produce Bid Form
        const bidForm = document.getElementById('produceBidForm');
        if (bidForm) {
            bidForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                const buyer = document.getElementById('bidBuyerName') ? document.getElementById('bidBuyerName').value : '';
                const quote = document.getElementById('bidPriceQuote') ? document.getElementById('bidPriceQuote').value : '';
                const phone = document.getElementById('bidPhone') ? document.getElementById('bidPhone').value : '';
                const crop = document.getElementById('inquiryCropName') ? document.getElementById('inquiryCropName').textContent : 'Produce';

                const endpoint = (typeof AgriServerData !== 'undefined' && AgriServerData.bidUrl) ? AgriServerData.bidUrl : '/wp-json/agri/v1/bid';

                try {
                    await fetch(endpoint, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ buyer_name: buyer, bid_price: quote, phone, crop })
                    });
                } catch (err) {}

                showToast('✅ Purchase quote submitted successfully to the seller!', 'success');
                ModalManager.close('produceInquiryModal');
                bidForm.reset();
            });
        }
    },
